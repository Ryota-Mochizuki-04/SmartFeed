...
class S3DataManager:
    ...
    def load_rss_config(self) -> Dict:
        ...
        try:
            response = self.s3_client.get_object(
                Bucket=self.bucket_name, Key="rss-list.json"
            )
            content = response["Body"].read().decode("utf-8")
            data = json.loads(content)
            self._validate_rss_config(data)
            self.logger.info(
                f"RSS設定を正常に読み込みました。フィード数: {len(data.get('feeds', []))}"
            )
            return data
        except ClientError as e:
            if e.response["Error"]["Code"] == "NoSuchKey":
                self.logger.info(
                    "RSS設定ファイルが存在しません。デフォルト設定を作成します。"
                )
    ...
    def load_notification_history(self) -> Dict:
        ...
        try:
            response = self.s3_client.get_object(
                Bucket=self.bucket_name, Key="notified-history.json"
            )
            content = response["Body"].read().decode("utf-8")
            data = json.loads(content)

            self.logger.info(
                f"通知履歴を正常に読み込みました。履歴数: {len(data.get('history', []))}"
            )
            return data
        except ClientError as e:
            if e.response["Error"]["Code"] == "NoSuchKey":
                self.logger.info(
                    "通知履歴ファイルが存在しません。デフォルト履歴を作成します。"
                )
    ...
    def get_recent_notifications(self, days: int = 7) -> List[Dict]:
        ...
        try:
            ...
            recent_notifications = [
                h for h in history["history"] if h["notified_at"] > cutoff_iso
            ]
        ...
        for i, feed in enumerate(data["feeds"]):
            ...
            for field in required_feed_fields:
                if field not in feed:
                    raise ValueError(
                        f"フィード{i}に必須フィールド'{field}'がありません"
                    )
    ...
    def _cleanup_history(self, history_data: Dict) -> None:
        ...
        if len(history_data["history"]) > max_size:
            ...
            self.logger.info(
                f"履歴サイズ制限により{deleted_count}件の古い履歴を削除しました"
            )
        ...
        history_data["history"] = [
            h for h in history_data["history"] if h["notified_at"] > cutoff_iso
        ]

        deleted_count = original_count - len(history_data["history"])
        if deleted_count > 0:
            self.logger.info(
                f"保持期間({cleanup_days}日)を超えた{deleted_count}件の履歴を削除しました"
            )
        ...
        if history_entries:
            ...
            if history_data["statistics"]["oldest_record"]:
                oldest_date = datetime.fromisoformat(
                    history_data["statistics"]["oldest_record"].replace("Z", "+00:00")
                )
                days_diff = (datetime.now(timezone.utc) - oldest_date).days
                if days_diff > 0:
                    avg_daily = len(history_entries) / days_diff
                    history_data["statistics"]["avg_daily_notifications"] = round(
                        avg_daily, 1
                    )
